...
Honestly, the site is not that complex. It's Next.js with a static export, so there's no backend at all. Everything ships as static files and Cloudflare serves them. The map itself is MapLibre GL with a custom parchment style I made so it looks like an old atlas instead of a modern street map.
